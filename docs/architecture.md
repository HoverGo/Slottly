...
### Company
Компания с владельцем (`owner_id`). Привязана к одной `UserSubscription` через `company_id` в подписке.
...
### CompanyService
Услуга компании: название, `duration_minutes` (5–480), опционально цена, привязка к специалисту или филиалу.
...
Промокоды, дашборд, списки пользователей/компаний — только админы.
